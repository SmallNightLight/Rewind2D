...
struct CollisionCheckInfo
{
    ...
    bool operator==(const CollisionCheckInfo& otherCollisionCheckInfo) const
    {
        return
            ...
            Position1 == otherCollisionCheckInfo.Position1 &&
            Position2 == otherCollisionCheckInfo.Position2 &&
            Rotation1 == otherCollisionCheckInfo.Rotation1 &&
            Rotation2 == otherCollisionCheckInfo.Rotation2 &&
...
struct CollisionCheckInfoHash
{
    std::size_t operator()( const CollisionCheckInfo& checkInfo ) const
    {
        ...
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Position1.X.raw_value()));
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Position1.Y.raw_value()));
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Position2.X.raw_value()));
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Position2.Y.raw_value()));
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Rotation1.raw_value()));
        hash = CombineHash(hash, static_cast<uint32_t>(checkInfo.Rotation2.raw_value()));
