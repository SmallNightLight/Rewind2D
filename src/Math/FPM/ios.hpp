...
template <typename CharT, typename B, typename I, typename T1, typename T2, unsigned int F, bool R>
std::basic_ostream<CharT>& operator<<(std::basic_ostream<CharT>& os, fixed<B,I,T1,T2,F,R> x) noexcept
{
...
template <typename CharT, class Traits, typename B, typename I, typename T1, typename T2, unsigned int F, bool R>
std::basic_istream<CharT, Traits>& operator>>(std::basic_istream<CharT, Traits>& is, fixed<B,I,T1,T2,F,R>& x)
{
    ...
    if (i > 0) {
        if (i == 3 || i == 8) {
            x = negate ? std::numeric_limits<fixed<B,I,T1,T2,F,R>>::min() : std::numeric_limits<fixed<B,I,T1,T2,F,R>>::max();
        } else {
    ...
    if (exponent_overflow) {
        ...
        if (std::all_of(significand.begin(), significand.end(), [](unsigned char x){ return x == 0; })) {
            ...
            x = fixed<B,I,T1,T2,F,R>(0);
        } else if (exponent_negate) {
            // A huge negative exponent approaches 0.
            x = fixed<B,I,T1,T2,F,R>::from_raw_value(0);
        } else {
            // A huge positive exponent approaches infinity.
            x = std::numeric_limits<fixed<B,I,T1,T2,F,R>>::max();
        }
        return is;
    }
    ...
    x = fixed<B,I,T1,T2,F,R>::from_raw_value(static_cast<B>(negate ? -raw_value : raw_value));
    return is;
}
